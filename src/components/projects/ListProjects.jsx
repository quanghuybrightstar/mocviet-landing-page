...
import { PROJECT_TAG_PILL_CLASS } from "@/libs/project-ui";
...
function ProjectListMeta({ project }) {
  ...
  return (
    <>
      ...
      <h3 className="m-0 font-semibold text-lg md:text-2xl text-heading group-hover:text-[var(--primary-color)] transition-colors duration-300">
        {project.title}
      </h3>
      {description ? (
        <p className="m-0 text-sm font-normal leading-5 text-secondary line-clamp-2 md:text-base md:leading-normal md:line-clamp-3">
          {description}
        </p>
      ) : null}
    </>
      ...
      {projects.map((project, index) => {
        ...
        return (
          <div
            ...
            className={clsx("relative min-w-0", {
              "item-dot": isLeftCol,
              "z-[2]": isLeftCol,
            })}
          >
            <FadeUp
              ...
              slideFromMd={isLeftCol ? "left" : "right"}
            >
              <Link
                prefetch
                ...
                className={clsx(
                  ...
                  "md:rounded-xl md:bg-surface-card md:shadow-sm md:hover:shadow-md md:transition-shadow md:duration-500",
                )}
              >
                <div
                  className={clsx(
                ...
                >
                  <Image
                    ...
                    src={
                      project.imageUrls?.[0] ||
                      "/images/projects/project_1.webp"
                    }
                    width={840}
                    height={840}
                  ...
                  <div className="absolute inset-0 z-[1] hidden md:flex bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 items-center justify-center pointer-events-none">
                    <span className="bg-white text-[#1A202C] text-sm font-semibold px-6 py-2.5 rounded-full flex items-center gap-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-300 shadow-sm">
                      Xem chi tiết
                      <Image
